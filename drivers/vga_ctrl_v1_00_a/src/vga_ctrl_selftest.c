/*****************************************************************************
* Filename:          D:\RA92-2016\lab3/drivers/vga_ctrl_v1_00_a/src/vga_ctrl_selftest.c
* Version:           1.00.a
...
XStatus VGA_CTRL_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < VGA_CTRL_USER_NUM_REG; write_loop_index++)
    VGA_CTRL_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < VGA_CTRL_USER_NUM_REG; read_loop_index++)
    if ( VGA_CTRL_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (VGA_CTRL_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
